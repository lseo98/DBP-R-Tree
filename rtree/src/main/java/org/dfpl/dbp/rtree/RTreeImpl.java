...
public class RTreeImpl implements RTree {
    ...
    public RTreeImpl(boolean enableVisualization) {
        ...
        this.visualizer = this.visualizationEnabled ? new RTreeVisualizer() : null;
    }
    ...
    public Iterator<Point> search(Rectangle rectangle) {
        ...
        List<Node> prunedNodes = new ArrayList<>();
        search(root, result, bounds[0], bounds[1], bounds[2], bounds[3], visitedNodes, prunedNodes);
        visualizeSearch(bounds, visitedNodes, prunedNodes, result);
    ...
    private void search(Node node, List<Point> result, double minX, double minY, double maxX, double maxY,
            List<Node> visitedNodes, List<Node> prunedNodes) {
        ...
        for (Node child : node.children) {
            search(child, result, minX, minY, maxX, maxY, visitedNodes, prunedNodes);
        }
    }
    ...
    private void visualizeSearch(double[] bounds, List<Node> visitedNodes, List<Node> prunedNodes,
            List<Point> resultPoints) {
        if (!visualizationEnabled) {
            return;
        }
        visualizer.update(root, "Search rectangle", bounds, visitedNodes, prunedNodes, resultPoints, null, null, null);
    }
